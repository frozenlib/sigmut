...
impl<S, T> SourceRefFrom<ObsCell<S>> for T
where
    S: AsRef<T> + 'static,
    T: ?Sized + 'static,
{
    fn source_ref_from(s: ObsCell<S>) -> SourceRef<Self> {
        (&s).into_source_ref()
    }
}
impl<S, T> SourceRefFrom<&ObsCell<S>> for T
...
{
    fn source_ref_from(s: &ObsCell<S>) -> SourceRef<Self> {
        s.obs().into_source_ref()
    }
}
impl<S, T> SourceRefFrom<ObsCollector<S>> for T
where
    S: Collect,
    S::Output: AsRef<T>,
    T: ?Sized,
{
    fn source_ref_from(s: ObsCollector<S>) -> SourceRef<Self> {
        (&s).into_source_ref()
    }
}
impl<S, T> SourceRefFrom<&ObsCollector<S>> for T
